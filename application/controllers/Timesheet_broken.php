...
class Timesheet extends MY_Controller
{
	...
	public function task_list()
	{
		...
		foreach ($task->result() as $r) {
			...
			/// set task progress
			if ($r->task_progress == '' || $r->task_progress == 0):
				$progress = 0;
			else:
	...
	public function attendance_list()
	{
		...
		foreach ($employee->result() as $r) {
			...
			if (empty($office_shift)) {
				$office_shift = array();
				$office_shift[0] = new stdClass();
				$office_shift[0]->monday_in_time = '';
				$office_shift[0]->monday_out_time = '';
				$office_shift[0]->tuesday_in_time = '';
				$office_shift[0]->tuesday_out_time = '';
				$office_shift[0]->wednesday_in_time = '';
				$office_shift[0]->wednesday_out_time = '';
				$office_shift[0]->thursday_in_time = '';
				$office_shift[0]->thursday_out_time = '';
				$office_shift[0]->friday_in_time = '';
				$office_shift[0]->friday_out_time = '';
				$office_shift[0]->saturday_in_time = '';
				$office_shift[0]->saturday_out_time = '';
				$office_shift[0]->sunday_in_time = '';
				$office_shift[0]->sunday_out_time = '';
			}
	...
	public function date_wise_list()
	{
		...
		$date_range_array = iterator_to_array($date_range);
		...
		echo "TYPE: " . gettype($date_range_array);
		exit;
		$output = array(
			"draw" => $draw,
			"recordsTotal" => count($date_range_array),
			"recordsFiltered" => count($date_range_array),
			"data" => $data
		);
		echo json_encode($output);
		exit();
	...
	public function holidays_list()
	{
		...
		foreach ($holidays->result() as $r) {
			...
			if ($r->is_publish == 1):
				$publish = $this->lang->line('xin_published');
			else:
				$publish = $this->lang->line('xin_unpublished');
			endif;
	...
	public function leave_list()
	{
		...
		foreach ($leave->result() as $r) {
			...
			elseif ($r->status == 2): $status = $this->lang->line('xin_approved');
			elseif ($r->status == 3):
