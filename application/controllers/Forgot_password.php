	...
	public function send_mail()
	{
		...
		if ($this->input->post('iemail')) {
			...
			if ($user > 0) {
				...
				// Always log for debugging on localhost
				$log_message = "Email Debug Info (Timestamp: " . date('Y-m-d H:i:s') . "):\n";
				$log_message .= "To: " . $this->input->post('iemail') . "\n";
				$log_message .= "Subject: " . $subject . "\n";
				$log_message .= "Message: " . $message . "\n";
				$log_message .= "--------------------------------------------------\n";
				file_put_contents(APPPATH . 'logs/email_debug.log', $log_message, FILE_APPEND);

				if ($this->email->send()) {
					$Return['result'] = $this->lang->line('xin_success_sent_forgot_password');
				} else {
					$Return['result'] = $this->lang->line('xin_success_sent_forgot_password') . " (Logged to logs/email_debug.log)";
				}
			} else {
